...
// applyPeople). These are real framework chromatic classes (bg--{hue}-45) — on a grayscale
// panel they automatically fall back to distinct perceptually-appropriate gray shades (no
// manual gray mapping needed), and render as actual color on a chromatic panel. Chip text
// stays solid black regardless of hue: step 45 (of the framework's 10=darkest/75=lightest
// scale) is light enough that black reads clearly against all 10 at once, so no per-hue
// foreground branching is needed for chips. A Person's badge circle is solid black/white
...
async function run(input) {
  ...
  for (const e of filtered) {
    const r = applyPeople(e.title, people);
    e.title = r.title;
    e.hueOverride = r.hue;
    e.badge = r.badge;
  }
...
//   {
//     "calendars": [
//       { "url": "https://.../family.ics", "color": "pink", "exclude": "regex" },
...
//     ]
//   }
// `calendars[].color` and `people[].color` are optional — one of HUES, pins that
...
// before people rules ever see them. `people` is global, not per-calendar — a person's
// `match` regex is tested against every surviving event from every calendar, in array order,
// each rule's rename building on the previous one's output (same chaining a plain find/replace
// had before people existed). A match renames the text to `name` (set `"rename": false` to
// tag/recolor without renaming) and, if `color`/`badge` are set, overrides that event's chip
// color and/or attaches a small badge (defaults to `name`'s first letter if `badge` is
// omitted) — last matching person wins for color/badge, same as color pinning above.
...
function parseConfig(raw) {
  ...
  for (const item of Array.isArray(data.calendars) ? data.calendars : []) {
    ...
    calendars.push({ url: item.url.trim(), color, exclude });
  }
  ...
  for (const item of Array.isArray(data.people) ? data.people : []) {
    ...
    people.push({ name, rx, color, badge, rename });
  }
...
function applyPeople(title, people) {
  let hue = null;
  let badge = null;
